...
RSpec.describe "show page", type: :feature do
  before :each do
    RentalProperty.create!(:title => "lovely", :description => "all you need to know", :price => 0.50, :maxpersons => 6, :latitude => 51.692139, :longitude => -3.317871)
    RentalProperty.create!(:title => "stinky", :description => "yup, it smells", :price => 2.30, :maxpersons => 4, :latitude => 35.807790, :longitude => -5.559082)
  ...
  it "should be able to create a new rental property" do
    ...
    expect(p.latitude.to_f).to be_within(1).of(42.8)
    expect(p.longitude.to_f).to be_within(1).of(-75)
  end
  ...
  it "should be able to create a new rental property" do
    ...
    p = RentalProperty.find(3)
    expect(p.latitude).to be_within(1).of(42.8)
    expect(p.longitude).to be_within(1).of(-75)
  end
