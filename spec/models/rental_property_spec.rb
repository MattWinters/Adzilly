...
RSpec.describe RentalProperty, type: :model do
  ...
  it "should fail to create a RentalProperty object if the lat/lng are not specified" do
      expect {
          RentalProperty.create!(title: "Pwll o le", description: "", bedrooms: 2, beds: 2, maxpersons: 4, bathrooms: 1, pets_allowed: false, address: "", price: 120.50)}.to raise_exception ActiveRecord::NotNullViolation
  end
  ...
  describe "filter_on_constraints tests" do
    before(:example) do
      1.upto(3) do |i|
        RentalProperty.create!(title: "Pwll o le #{i}", bedrooms: "#{i}", beds: "#{i*2}", maxpersons: "#{i*4}", bathrooms: "#{i}", pets_allowed: i%2==0, price: i*10.25, latitude: 0, longitude: 0)
      end
    end
